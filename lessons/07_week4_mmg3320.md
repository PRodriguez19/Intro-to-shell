...
9. Skip a line. On line 10 copy-and-paste the following comment. 
    ...
    #### Now you will move on to create a prefix for all (6) original fastq files. These prefixes will then be stored in second variable called `samplename`. To write this line of code successfully, 
...
10. On line 11 create the variable `samplename`
    + The value of the variable should be equal to the `basename` of first variable you created above!
    + Remember to recall a variable you will need `$`
...
11. Copy-and-paste the following into lines 12 and 13. The `echo` statement will keep the user informed on which file is being processed. 
...
Take a look inside the `badreads` directory, you should see that for every one of the original FASTQ files have one bad read file. You should also have a summary file documenting the total number of bad reads from each file.
