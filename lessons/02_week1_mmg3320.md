...
In this case we are using a relative path, relative to our current location - wherein we know that the `raw_fastq` folder is within `unix_lesson` and `data` is within the `raw_fastq` folder. 
